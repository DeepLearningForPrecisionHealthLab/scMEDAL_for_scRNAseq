...
# Change this to specific batches
# batches_to_select_from = None
batches_to_select_from = ["AML420B","AML556","BM5","BM3","MUTZ3"]
...
cm_multibatch_name = f"CMmultibatch_{n_cells_per_batch}_cells_per_batch_{n_batches}batches" if not celltype_name else f"CMmultibatch_{n_cells_per_batch}_cells_per_batch_{n_batches}batches_{celltype_name}"
...
if isinstance(celltype, list):
    ...
    if batches_to_select_from is None:
        # Select here how many batches do you want to plot. It will be one per column.
    ...
    cell_ids_2plot = select_cells_from_batches(obs_multibatch, celltype, batches_to_select_from, seed=seed,cell_id_col=cell_id_col)
    # Here, the n cells to plot are different to the number of original batches. One batch can have one cell from every celltype
...
for cell_id in cell_ids_2plot:
    ...
    original_celltype = obs_multibatch.loc[(obs_multibatch[cell_id_col] ==cell_id)&(obs_multibatch["recon_prefix"]=="input_train"),"celltype"].values[0]
    ...
    plot_cell_recon_genomap(genomap,
                                ...
                                order='C',
                                path_2_genomap=path_2_genomap,
                                file_name=f"{cell_id}_few_batches_{statistic}_{original_celltype}_genelabels",
                                remove_ticks=True)
