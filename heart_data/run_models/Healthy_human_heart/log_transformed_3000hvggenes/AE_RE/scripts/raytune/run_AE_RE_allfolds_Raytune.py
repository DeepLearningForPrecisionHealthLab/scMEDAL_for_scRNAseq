...
from model_config import RAY_RUN,build_model_dict,data_seen,outputs_path, save_model, folder_name, model_name,model_params_dict,compile_dict,data_path
# path to models
...
#RAY_RUN = True
# 0. Define Fold
folds_list = list(range(1,6,1)) #there are 5 folds. Running 2 for testing
...
class Trial(tune.Trainable):
    ...
    def step(self):
        ...
        from model_train_utils import run_all_folds,get_metric2optimize_re#,generate_run_name,
        from model_config import compile_dict,build_model_dict,load_data_dict,train_model_dict,get_scores_dict,save_model,data_seen#,base_paths_dict#, constant_keys, 
        from AE_v4 import DomainEnhancingAutoencoderClassifier


        outputs_path = self.logdir
        ...
        run_name_step = outputs_path.split("/")[-1]
        print("run_name",run_name_step)
...
if not RAY_RUN:
    ...
    num_GPU = 1
    num_CPU = 16 #CHANGED BECAUSE OF MEMORY issues
    os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = 'true'
...
if not RAY_RUN:
    ...
    tune.run(
        ...
        max_concurrent_trials=16  # Limit to 2 concurrent trials (to avoid memory errors)
    )
