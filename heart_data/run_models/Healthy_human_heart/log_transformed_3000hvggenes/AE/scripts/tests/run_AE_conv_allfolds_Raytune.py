...
sys.path.append("/archive/bioinformatics/DLLab/AixaAndrade/src/ARMED_genomics_git/heart_data/run_models/Healthy_human_heart/log_transformed_3000hvggenes/AE")
#from model_config import *
from model_config import build_model_dict,data_seen,run_name,base_paths_dict,outputs_path, save_model,function_type, folder_name, model_name,model_params_dict,compile_dict,data_path
# path to models
...
class Trial(tune.Trainable):
    ...
    def step(self):
        ...
        from model_train_utils import run_all_folds,generate_run_name,get_metric2optimizemodel
        ...
        # Run all folds
        mean_scores_dict = run_all_folds(Model=AE,
                input_base_path=data_seen,
                out_base_paths_dict=base_paths_dict,# This is dict
                folds_list=folds_list, #there are 10 folds for just running 2 for testing
                run_name=run_name,
                model_params_dict=model_params_dict_step,#This is what is really getting optimized
        ...
        print(mean_scores_dict)
        # maximizing biological clustering and minimizing batch clustering
        metric2optimize = get_metric2optimizemodel(mean_scores_dict, subset='val', metric='silhouette', batch_col=batch_col, bio_col=bio_col)
...
if not RAY_RUN:
    ...
    ray_base = os.path.join(outputs_path, "ray", function_type, folder_name, model_name)
    ...
    search_space = {
        ...
        "build_model_params":{}, # Update params from build_model_dict that you want to tune
        "batch_size": tune.grid_search([32, 64, 128, 256,512]) # I am using grid_search because there is just a few options for batch_size
    ...
    # FIFOSchedulerscheduler is compatible with grid search. I am only training batch size with very few options. Thus, it is better to use grid search instead of BOHB
