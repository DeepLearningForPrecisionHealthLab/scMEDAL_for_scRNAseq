...
def run_scvi_pipeline(
    ...
    model = Model(
        ...
        use_observed_lib_size=False,   # learn posterior,
    )
    ...
    def _extract_and_store(ad: AnnData, data_type: str) -> None:
        scvi.model.SCVI.setup_anndata(ad, batch_key=batch_col)       # make sure the manager is registered
        latent = model.get_latent_representation(ad)                 # (n_cells, n_latent)
        libsize = model.get_latent_library_size(ad, give_mean=True)  # (n_cells,)  ? the scaling factor ??

        # 1. attach to AnnData (use whatever key you like)
        ad.obsm["X_scvi"]           = latent
        ad.obs["scvi_scale_factor"] = libsize

        # 2. save to disk beside the other arrays
        np.save(os.path.join(model_params.latent_path,
                            f"{model_params.encoder_latent_name}_{data_type}.npy"), latent)
        np.save(os.path.join(model_params.latent_path,
                            f"scale_factor_{data_type}.npy"), libsize)

        # 3. (unchanged) save denoised / batch-corrected expression
...
def run_scvi_across_folds(
    ...
    if return_scores_temp and all_scores_per_fold:
        ...
        for data_split in ["train", "val", "test"]:
            ...
            #     index=False
            # )
            # keep only columns that are numeric **right now**
            numeric_cols = big_df.select_dtypes(include="number").columns

            # mean_df = (
            #     big_df.groupby(big_df["dataset_type"])[numeric_cols]   # group by the string column
            #         .mean()                                           # numeric_only implied
            #         .reset_index()                                    # optional: keep column
            # )
            ...
            # Combine mean, std, and SEM into a single DataFrame with multi-level columns
            # summary_df = pd.concat([mean_scores, std_scores, sem_scores], axis=0, keys=['mean', 'std', 'sem'])

            # # Transpose the DataFrame without flattening the multi-level columns
            # summary_df_transposed = summary_df.T

            # # Save summary DataFrame (transposed)
...
#                    dataset_type: str) -> pd.DataFrame:
#     """
#     df_scores has MultiIndex columns (label, metric) after
#     restructure_dataframe / calculate_merge_scores.
#     """
#     row = {}
#     metrics = ["ch", "1/db", "silhouette"]

#     # take the first latent row (or iterate over latents if you prefer)
#     first = df_scores.iloc[0]

#     for lbl in labels:
#         for m in metrics:
#             row[f"{lbl}_{m}"] = first[(lbl, m)]
