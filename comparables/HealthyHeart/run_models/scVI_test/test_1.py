...
def run_scvi_across_folds(
    ...
    # if return_scores_temp and all_scores_per_fold:          # only if scores requested
    #     splits_to_do = ["train", "val", "test"]
    #     for split_name in splits_to_do:

    #         # skip TEST if the experiment didn't have a test set
    #         if split_name == "test" and not model_params.eval_test:
    #             continue

    #         per_fold_dfs = []
    #         for fold_idx, split_dict in all_scores_per_fold.items():

    #             # safety: some folds might not carry this split (e.g. no test)
    #             if split_name not in split_dict:
    #                 continue

    #             df_tmp = split_dict[split_name].copy()      # DataFrame from run_scvi_pipeline
    #             # prepend the fold label to the columns so we can merge side?by?side
    #             df_tmp.columns = pd.MultiIndex.from_product(
    #                 [[f"fold{fold_idx}"], df_tmp.columns]
    #             )
    #             per_fold_dfs.append(df_tmp)

    #         if not per_fold_dfs:        # nothing collected for this split
    #             continue

    #         # --------------- concatenate & save ----------------
    #         big_df = pd.concat(per_fold_dfs, axis=1)

    #         big_df.to_csv(
    #             os.path.join(
    #                 model_params.latent_path_main,
    #                 f"all_scores_{split_name}_samplesize-{sample_size}.csv",
    #             )
    #         )

    #         # --------------- compute fold?mean & save ----------
    #         mean_df = big_df.groupby(level=1, axis=1).mean()
    #         mean_df.to_csv(
    #             os.path.join(
    #                 model_params.latent_path_main,
    #                 f"mean_scores_{split_name}_samplesize-{sample_size}.csv",
    #             )
    #         )

    #         # store for optional downstream use
    #         merged_scores[split_name] = mean_df
