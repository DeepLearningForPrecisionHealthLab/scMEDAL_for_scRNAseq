...
def _extract_and_store(ad: AnnData, data_type: str, model_params=None):
    ...
    """
    # Save Scanorama latent (as encoder_latent_name_<split>.npy)
    ...
    # Save PCA latent (as pca_latent_<dims>_<split>.npy)
    pca_latent_key = f"pca_latent_{latent.shape[1]}_{data_type}"
    ad.obsm[pca_latent_key] = ad.obsm["X_pca"]
    np.save(os.path.join(model_params.latent_path, f"{pca_latent_key}.npy"), ad.obsm["X_pca"])
    
    # Save reconstructed counts (Scanorama-corrected ad.X)
    recon = ad.X
...
def run_scanorama_pipeline(
    ...
    compile_dict: Dict[str, Any],  # unused, kept for compatibility
    model_params,
    save_model: bool,
    batch_col: str,
    bio_col: str,
    ...
    model_type: str = "scanorama",  # placeholder
    issparse: bool = False,
    ...
    """
    Scanorama CV pipeline: Concatenate splits, integrate, split back, save latents and recon.
    Keeps variable names and outputs identical to scVI pipeline for drop-in replacement.
    """
    ...
    for split_name in ["train", "val", "test"]:
        if split_name in adata_dict:
            ...
            split_data.append(adata_dict[split_name])
    # Remove pre-existing "split" column if present
    for ad in split_data:
        if "split" in ad.obs.columns:
            ad.obs = ad.obs.drop(columns=["split"])
    ...
    # Make sure all batch values are strings (Scanorama expects this)
    adata_full.obs[batch_col] = adata_full.obs[batch_col].astype(str)

    # Sort so that batches are contiguous
    adata_full = adata_full[adata_full.obs[batch_col].argsort()].copy()

    # 3. PCA and Scanorama on all data
    # since here we dont have hidden layers but a pre pca step
    sc.pp.pca(adata_full, n_comps=model_params.n_latent_dims)

    # # Ensure batch_col is string
    adata_full.obs[batch_col] = adata_full.obs[batch_col].astype(str)

    # Integrate with Scanpy wrapper
    sce.pp.scanorama_integrate(adata_full, key=batch_col)

    # Split integrated object into original splits
    adata_train = adata_full[adata_full.obs["split"] == "train"].copy()
    adata_val = adata_full[adata_full.obs["split"] == "val"].copy()
    adata_test = adata_full[adata_full.obs["split"] == "test"].copy() \
                if "test" in adata_full.obs["split"].unique() else None


    # Prepare final dict
    adata_dict = {"train": adata_train, "val": adata_val}
    if adata_test:
        adata_dict["test"] = adata_test

    # 5. Save Scanorama latent, PCA latent, and reconstructed counts using your conventions
    os.makedirs(model_params.latent_path, exist_ok=True)
    _extract_and_store(adata_train, "train", model_params)
    _extract_and_store(adata_val, "val", model_params)
    if adata_test is not None:
        _extract_and_store(adata_test, "test", model_params)

    # 6. Optional: clustering/batch metrics (identical to scVI pipeline)
    df_scores_dict = None
    if return_scores:
        df_scores_dict = {}
        for split, ad in adata_dict.items():
            print(f"Computing clustering scores for {split}")
            # NOTE: uses new latent keys as well as PCA, both in .obsm now
            latent_list = [
    ...
    if return_history:
        results["history"] = pd.DataFrame()  # No history for Scanorama
