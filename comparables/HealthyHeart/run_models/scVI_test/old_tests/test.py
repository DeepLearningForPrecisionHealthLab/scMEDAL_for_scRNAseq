...
def run_scvi_pipeline(
    ...
    df_scores_dict = None
    if return_scores:
        from scMEDAL.utils.model_train_utils import compute_scores  # local import to avoid circular deps
        df_scores_dict = {}
...
def run_scvi_across_folds(
    ...
    for intFold in folds_list:
        ...
        all_history_df = pd.concat(
            [all_history_df, fold_results["history"].assign(fold=intFold)],
            ignore_index=True,
        )
        gc.collect()

    # 5) Aggregate scores if requested
    mean_scores = None
    if return_scores_temp:
        mean_scores = {
            split: pd.concat(dfs).groupby(level=0).mean() for split, dfs in all_scores.items()
        }
...
def plot_rep(
    ...
    figsize: tuple = (7, 7)):
    """Plot 2‑D embedding with separate colour and shape legends."""
...
plot_params = {
        "markers":        ["o", "v", "^", "<", "*"],
        "save_fig":       True,
        "outpath":        "",  # set your output directory
        "showplot":       False,
        "palette_choice": "tab20",
    }

postprocess_cv_results(cv_results, shape_color_dict, plot_params)
